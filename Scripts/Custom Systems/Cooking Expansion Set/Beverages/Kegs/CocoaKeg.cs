...
{
	public class CocoaKeg : Item, ICraftable
	{
		...
		public CocoaKeg( Serial serial ) : base( serial )
		{ }
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if (this.Name == null )
			{
				...
				else this.LabelTo( from, "Keg of Cocoa" );
			}
			...
			if ( AllowBottling )
			{
				if ( from.InRange( GetWorldLocation(), 2 ) )
				{
					if ( m_Held > 0 )
					{
						...
						{
							from.SendMessage( "You fill the mug with cocoa" );
							BaseCraftCocoa cocoa = FillBottle();
							cocoa.Crafter = m_Crafter;
							cocoa.Quality = m_Quality;
							cocoa.Variety = m_Variety;
							if (this.Name != null) cocoa.Name = this.Name;
							if ( pack.TryDropItem( from, cocoa, false ) )
							{
		...
		public BaseCraftCocoa FillBottle()
		{
			switch ( m_Variety )
			{
				default: return new MugOfCocoa();
			}
		}
