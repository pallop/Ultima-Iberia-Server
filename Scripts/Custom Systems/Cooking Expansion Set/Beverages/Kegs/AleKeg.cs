...
namespace Server.Items
{
	public class AleKeg : Item, ICraftable
	{
		...
		public AleKeg( Serial serial ) : base( serial ) { }
		...
		public override void AddNameProperties( ObjectPropertyList list )
 		{
			...
			if ( m_Quality == AleQuality.Exceptional ) list.Add( 1060847, "Black Label\t{0}", aleType );
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if (this.Name == null )
			{
				...
				else this.LabelTo( from, "Cheap Ale" );
			}
			else this.LabelTo( from, "{0}", this.Name );
			aleType = BrewingResources.GetName( m_Variety );
			if ( m_Quality == AleQuality.Exceptional ) this.LabelTo( from, "Black Label {0}", aleType );
				...
				if ( from.InRange( GetWorldLocation(), 2 ) )
				{
					if ( m_Held > 0 )
					{
						...
						if ( pack != null && pack.ConsumeTotal( typeof( EmptyAleBottle ), 1 ) )
						{
							...
							if (this.Name != null) ale.Name = this.Name;
		...
		public BaseCraftAle FillBottle()
		{
			switch ( m_Variety )
			{
				default: return new BottleOfAle();
			}
		}
