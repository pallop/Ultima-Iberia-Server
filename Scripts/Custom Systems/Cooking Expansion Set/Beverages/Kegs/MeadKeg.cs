...
namespace Server.Items
{
	public class MeadKeg : Item, ICraftable
	{
		...
		public MeadKeg( Serial serial ) : base( serial )
		{ }
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( GetSaveFlag( flags, SaveFlag.Quality ) ) m_Quality = (MeadQuality)reader.ReadEncodedInt();
			else m_Quality = MeadQuality.Regular;
			if ( m_Quality == MeadQuality.Low ) m_Quality = MeadQuality.Regular;
		...
		public override void AddNameProperties( ObjectPropertyList list )
 		{
			...
			if ( m_Quality == MeadQuality.Exceptional ) list.Add( 1060847, "Black Label\t{0}", meadType );
 			else list.Add( 1060847, "\t{0}", meadType );
 		}
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if (this.Name == null )
			{
				...
				else this.LabelTo( from, "Cheap Mead" );
			}
			else this.LabelTo( from, "{0}", this.Name );
			meadType = BrewingResources.GetName( m_Variety );
			if ( m_Quality == MeadQuality.Exceptional ) this.LabelTo( from, "Black Label {0}", meadType );
 			else this.LabelTo( from, "{0}", meadType );
			if ( m_Held <= 0 ) number = 502246;
			else if ( m_Held < 5 ) number = 502248;
			else if ( m_Held < 10 ) number = 502249;
			else if ( m_Held < 18 ) number = 502250;
				...
				if ( from.InRange( GetWorldLocation(), 2 ) )
				{
					if ( m_Held > 0 )
					{
						...
						if ( pack != null && pack.ConsumeTotal( typeof( EmptyMeadBottle ), 1 ) )
						{
							...
							if (this.Name != null) mead.Name = this.Name;
		...
		public BaseCraftMead FillBottle()
		{
			switch ( m_Variety )
			{
				default: return new BottleOfMead();
			}
		}
		...
		public int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue )
		{
			...
			if ( Quality == MeadQuality.Exceptional ) Crafter = from;
