...
namespace Server.Items
{
	public class CoffeeKeg : Item, ICraftable
	{
		...
		private CoffeeQuality m_Quality;
		...
		public CoffeeKeg( Serial serial ) : base( serial )
		{ }
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			SetSaveFlag( ref flags, SaveFlag.Quality, m_Quality != CoffeeQuality.Regular );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( GetSaveFlag( flags, SaveFlag.Quality ) ) m_Quality = (CoffeeQuality)reader.ReadEncodedInt();
			else m_Quality = CoffeeQuality.Regular;
			if ( m_Quality == CoffeeQuality.Low ) m_Quality = CoffeeQuality.Regular;
		...
		public override void AddNameProperties( ObjectPropertyList list )
 		{
			...
			if ( m_Quality == CoffeeQuality.Exceptional ) list.Add( "Reserve Dark Roast Coffee" );
 			else list.Add( "Black Coffee" );
 		}
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if (this.Name == null )
			{
				...
				else this.LabelTo( from, "Keg of Coffee" );
			}
			else this.LabelTo( from, "{0}", this.Name );
			if ( m_Quality == CoffeeQuality.Exceptional ) this.LabelTo( from, "Reserve Dark Roast Coffee" );
 			else this.LabelTo( from, "Black Coffee" );
			if ( m_Held <= 0 ) number = 502246;
			else if ( m_Held < 5 ) number = 502248;
			else if ( m_Held < 10 ) number = 502249;
			else if ( m_Held < 18 ) number = 502250;
			...
			if ( AllowBottling )
			{
				if ( from.InRange( GetWorldLocation(), 2 ) )
				{
					if ( m_Held > 0 )
					{
						...
						{
							from.SendMessage( "You fill the mug with coffee" );
							BaseCraftCoffee coffee = FillBottle();
							coffee.Crafter = m_Crafter;
							coffee.Quality = m_Quality;
							coffee.Variety = m_Variety;
							if (this.Name != null) coffee.Name = this.Name;
							if ( pack.TryDropItem( from, coffee, false ) )
							{
		...
		public BaseCraftCoffee FillBottle()
		{
			switch ( m_Variety )
			{
				default: return new MugOfCoffee();
			}
		}
		...
		public int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue )
		{
			...
			if ( Quality == CoffeeQuality.Exceptional ) Crafter = from;
