...
namespace Server.Items
{
	public class TeaKeg : Item, ICraftable
	{
		...
		public TeaKeg( Serial serial ) : base( serial )
		{ }
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			SetSaveFlag( ref flags, SaveFlag.Quality, m_Quality != TeaQuality.Regular );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if ( GetSaveFlag( flags, SaveFlag.Quality ) ) m_Quality = (TeaQuality)reader.ReadEncodedInt();
			else m_Quality = TeaQuality.Regular;
			if ( m_Quality == TeaQuality.Low ) m_Quality = TeaQuality.Regular;
		...
		[Constructable]
		public TeaKeg( ) : base( 0x1940 )
		{
			this.Weight = 1.0;
			m_Held = 75;
			m_Quality = TeaQuality.Regular;
			m_Crafter = null;
		...
		public override void AddNameProperties( ObjectPropertyList list )
 		{
			...
			if ( m_Quality == TeaQuality.Exceptional ) list.Add( "Reserve Sweet Green Tea" );
 			else list.Add( "Green Tea" );
 		}
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			if (this.Name == null )
			{
				...
				else this.LabelTo( from, "Keg of Tea" );
			}
			else this.LabelTo( from, "{0}", this.Name );
			if ( m_Quality == TeaQuality.Exceptional ) this.LabelTo( from, "Reserve Sweet Green Tea" );
 			else this.LabelTo( from, "Green Tea" );
			if ( m_Held <= 0 ) number = 502246;
			else if ( m_Held < 5 ) number = 502248;
			else if ( m_Held < 10 ) number = 502249;
			else if ( m_Held < 18 ) number = 502250;
			...
			if ( AllowBottling )
			{
				if ( from.InRange( GetWorldLocation(), 2 ) )
				{
					if ( m_Held > 0 )
					{
						...
						if ( pack != null && pack.ConsumeTotal( typeof( PewterMug ), 1 ) )
						{
							from.SendMessage( "You fill the mug with tea" );
							BaseCraftTea tea = FillBottle();
							tea.Crafter = m_Crafter;
							tea.Quality = m_Quality;
							tea.Variety = m_Variety;
							if (this.Name != null) tea.Name = this.Name;
							if ( pack.TryDropItem( from, tea, false ) )
							{
		...
		public BaseCraftTea FillBottle()
		{
			switch ( m_Variety )
			{
				default: return new MugOfTea();
			}
		}
		...
		{
			Held = 75;
			Quality = (TeaQuality)quality;
			if ( Quality == TeaQuality.Exceptional ) Crafter = from;
