...
namespace Server.Items.Crops
{
	...
	public class SquashSeedling : BaseCrop
	{
		...
		{
			plant.thisTimer = new CropHelper.GrowTimer( plant, typeof(SquashCrop), plant.Sower );
	...
	public class SquashCrop : BaseCrop
	{
		...
		[Constructable]
		public SquashCrop( Mobile sower ) : base( 15374 )
		{
			Movable = false;
			Name = "Squash Plant";
			Hue = 2995;
		...
		private class CropTimer : Timer
		{
			...
			public CropTimer( SquashCrop plant ) : base( TimeSpan.FromSeconds( 450 ), TimeSpan.FromSeconds( 15 ) )
			{
		...
		public void Chop( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( from == m_sower )
				{
					...
					{
						Squash fruit = new Squash( Utility.Random( m_yield +2 ) );
