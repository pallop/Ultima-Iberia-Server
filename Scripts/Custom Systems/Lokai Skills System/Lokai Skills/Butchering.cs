...
namespace Server.LokaiSkillHandlers
{
    public class Butchering
    {
        ...
        {
            LokaiSkillInfo.Table[(int)LokaiSkillName.Butchering].Callback = new LokaiSkillUseCallback(OnUse);
        }

        public static TimeSpan OnUse(Mobile m)
        {
            Item item = m.FindItemOnLayer(Layer.OneHanded);
            if (item != null && (item is ButcherKnife || item is Cleaver))
            {
            ...
            else
            {
                ...
                if (pack != null
                    && (pack.FindItemByType(new Type[] { typeof(ButcherKnife), typeof(Cleaver) }) != null))
                {
                    m.Target = new InternalTarget();
                }
                else
                    m.SendMessage("You need a Butcher's knife or a Cleaver to use this skill.");
            }
        ...
        public static void OnCarve(Mobile from, Corpse corpse, BaseCreature mob, LokaiSkill lokaiSkill)
        {
            if (corpse.Carved)
            {
                from.SendMessage("This corpse has already been carved up.");
                return;
            }
            ...
            {   //NO Hides bonus when butchering
                ...
                switch (rating)
                {
                    case SuccessRating.CriticalFailure: meatFactor = 3; ratingFactor = 0; break;
                    case SuccessRating.HazzardousFailure: meatFactor = 6; ratingFactor = 0; break;
                    case SuccessRating.Failure: meatFactor = 9; ratingFactor = 0; break;
                    case SuccessRating.PartialSuccess: meatFactor = 11; ratingFactor = 0; break;
                    case SuccessRating.Success: meatFactor = 14; ratingFactor = 3; break;
                    case SuccessRating.CompleteSuccess: meatFactor = 17; ratingFactor = 6; break;
                    case SuccessRating.ExceptionalSuccess:
                    case SuccessRating.TooEasy: meatFactor = 20; ratingFactor = 9; break;
                    default:
                    case SuccessRating.TooDifficult: meatFactor = 0; ratingFactor = 0; break;
                }

                feathers *= ratingFactor;
                wool *= ratingFactor;
                hides *= ratingFactor;
                scales *= ratingFactor;
                meat *= meatFactor;
                ...
                if (hides != 0)
                {
                    ...
                    from.SendMessage("You butcher it, and some hides are now in the corpse.");
