...
public ArrayList Barracks
		{
			get{ return m_Barracks; }
			set{ m_Barracks = value; }
		}
...
if ( this.Barracks.Count >= 1 )
			{
				int barrackCost = this.Barracks.Count * 1000;
				cost += barrackCost;
			}
...
if ( sign.Type == CivicSignType.Barracks )
										this.Barracks.Remove( sign );
				...
				case 3:
					{
							...
							for ( int i = 0; i < toDelete.Count; i++ )
							{
								if ( toDelete[i] is CivicSign )
								{
									...
									CivicSign sign = toDelete[i] as CivicSign;
									if ( sign.Type == CivicSignType.Bank || sign.Type == CivicSignType.Moongate || sign.Type == CivicSignType.Market || sign.Type == CivicSignType.Barracks) 
										continue;
				...
				case 4:
					{
							...
							for ( int i = 0; i < toDelete.Count; i++ )
							{
								if ( toDelete[i] is CivicSign )
								{
									...
									if ( sign.Type == CivicSignType.Bank || sign.Type == CivicSignType.Moongate || sign.Type == CivicSignType.Healer || sign.Type == CivicSignType.Garden || sign.Type == CivicSignType.Market || sign.Type == CivicSignType.Barracks)
										continue;
				...
				case 5:
					{
							...
							for ( int i = 0; i < toDelete.Count; i++ )
							{
								if ( toDelete[i] is CivicSign )
								{
									...
									if ( sign.Type == CivicSignType.Bank || sign.Type == CivicSignType.Moongate || sign.Type == CivicSignType.Healer || sign.Type == CivicSignType.Garden || sign.Type == CivicSignType.Stable || sign.Type == CivicSignType.Market || sign.Type == CivicSignType.Barracks)
										continue;
...
if ( this.Barracks == null )
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			UpdateRegion();
			//tru Update
                        writer.WriteItemList( m_Barracks, true );
                        writer.Write( m_General );
			//2.1.1
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 6:
					{
                        m_Barracks = reader.ReadItemList();
                        m_General = reader.ReadMobile();
