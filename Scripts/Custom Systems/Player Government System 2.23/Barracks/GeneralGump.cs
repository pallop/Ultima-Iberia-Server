...
namespace Server.Gumps
{
	public class GeneralGump : Gump
	{
		...
		public GeneralGump( CityManagementStone stone, int listPage, ArrayList list, ArrayList count ) : base( 50, 50 )
		{
			m_Stone = stone;
         		m_List = list;
         		m_ListPage = listPage;   
			...
			if ( from == null )
        		...
        		if ( info.ButtonID == 1 ) // Previous page
         		{
         			if ( m_ListPage > 0 )
					from.SendGump( new GeneralGump( m_Stone, m_ListPage - 1, m_List, m_CountList ) );
			}
        		...
        		if ( info.ButtonID >= 100 ) // Pick General
         		{
				...
				{
					((PlayerMobile)m_Stone.General).CityTitle = "Citizen";
				}
				
				pm.CityTitle = "General";
				m_Stone.General = mob;
				from.SendMessage( "You have selected {0} as your General.", mob.Name );
				from.CloseGump( typeof (GeneralGump) );
				from.SendGump( new CityManagementGump( m_Stone, from ) );
