	...
	public abstract class CityDeed : Item
	{
		...
		public void FinishPlacement( Mobile from, Point3D p )
		{
			if ( Type == CivicStrutureType.FieldStoneCityHall )
			{
              ...
              ArrayList barracks = new ArrayList();
...
mStone.AddOns = addon;
			...
			else if ( Type == CivicStrutureType.PlasterMarket )
			{
				...
				//Murallas
				else if ( Type == CivicStrutureType.EmpalizadaEscaleraNorte )
			{
				
			
			EmpalizadaEscaleraNorteAddon building = new EmpalizadaEscaleraNorteAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3021;
				sign.Name = "Empalizada Escalera Norte";
				sign.MoveToWorld( new Point3D( p.X +2, p.Y -5, p.Z + 10  ), from.Map );
			}

else if ( Type == CivicStrutureType.EmpalizadaNorte )
			{
				
			
			EmpalizadaNorteAddon building = new EmpalizadaNorteAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3021;
				sign.Name = "Empalizada  Norte";
				sign.MoveToWorld( new Point3D( p.X +2, p.Y -5, p.Z + 10  ), from.Map );
			}
			else if ( Type == CivicStrutureType.EmpalizadaPuertaN )
			{
				
			
			EmpalizadaPuertaNAddon building = new EmpalizadaPuertaNAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3021;
				sign.Name = "Empalizada Puerta Norte";
				sign.MoveToWorld( new Point3D( p.X +2, p.Y -5, p.Z + 17  ), from.Map );
			}


else if ( Type == CivicStrutureType.EmpalizadaTorreEsquinera )
			{
				
			
			EmpalizadaTorreEsquineraAddon building = new EmpalizadaTorreEsquineraAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3021;
				sign.Name = "Empalizada Torre Esquinera";
				sign.MoveToWorld( new Point3D( p.X , p.Y -5, p.Z + 50  ), from.Map );
			}

			else if ( Type == CivicStrutureType.EmpalizadaEscaleraWest )
			{
				
			
			EmpalizadaEscaleraWestAddon building = new EmpalizadaEscaleraWestAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3022;
				sign.Name = "Empalizada Escalera Oeste";
				sign.MoveToWorld( new Point3D( p.X , p.Y -3, p.Z + 7  ), from.Map );
			}

			else if ( Type == CivicStrutureType.EmpalizadaWest )
			{
				
			
			EmpalizadaWestAddon building = new EmpalizadaWestAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3022;
				sign.Name = "Empalizada  Oeste";
				sign.MoveToWorld( new Point3D( p.X , p.Y -3, p.Z + 7  ), from.Map );
			}


	else if ( Type == CivicStrutureType.EmpalizadaEst )
			{
				
			
			EmpalizadaEstAddon building = new EmpalizadaEstAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3022;
				sign.Name = "Empalizada  Este";
				sign.MoveToWorld( new Point3D( p.X +3 , p.Y -2, p.Z + 17  ), from.Map );
			}

			else if ( Type == CivicStrutureType.EmpalizadaEscaleraEst )
			{
				
			
			EmpalizadaEscaleraEstAddon building = new EmpalizadaEscaleraEstAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3022;
				sign.Name = "Empalizada Escalera  Este";
				sign.MoveToWorld( new Point3D( p.X +3 , p.Y -2, p.Z + 17   ), from.Map );
			}

			else if ( Type == CivicStrutureType.EmpalizadaEscaleraSur )
			{
				
			
			EmpalizadaEscaleraSurAddon building = new EmpalizadaEscaleraSurAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3021;
				sign.Name = "Empalizada Escalera Sur";
				sign.MoveToWorld( new Point3D(  p.X +2, p.Y -5, p.Z + 7 ), from.Map );
			}

			else if ( Type == CivicStrutureType.EmpalizadaSur )
			{
				
			
			EmpalizadaSurAddon building = new EmpalizadaSurAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3021;
				sign.Name = "Empalizada  Sur";
				sign.MoveToWorld( new Point3D(  p.X +2, p.Y -5, p.Z + 7 ), from.Map );
			}

else if ( Type == CivicStrutureType.EmpalizadaPuertaEst )
			{
				
			
			EmpalizadaPuertaEstAddon building = new EmpalizadaPuertaEstAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3022;
				sign.Name = "Empalizada Puerta Este";
				sign.MoveToWorld( new Point3D(  p.X +2, p.Y -5, p.Z + 10 ), from.Map );
			}
else if ( Type == CivicStrutureType.EmpalizadaPuertaWest )
			{
				
			
			EmpalizadaPuertaWestAddon building = new EmpalizadaPuertaWestAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3022;
				sign.Name = "Empalizada Puerta Oeste";
				sign.MoveToWorld( new Point3D(  p.X +2, p.Y -4, p.Z + 10 ), from.Map );
			}
else if ( Type == CivicStrutureType.EmpalizadaPuertaSur )
			{
				
			
			EmpalizadaPuertaSurAddon building = new EmpalizadaPuertaSurAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				...
				sign.ItemID = 3022;
				sign.Name = "Empalizada Puerta Sur";
				sign.MoveToWorld( new Point3D(  p.X +3, p.Y -5, p.Z + 9 ), from.Map );
			}
...
else if ( Type == CivicStrutureType.Barracks )
			{
                                
				BarracksAddon building = new BarracksAddon();
				building.MoveToWorld( new Point3D( p.X - m_Offset.X, p.Y - m_Offset.Y, p.Z - m_Offset.Z ), from.Map );
				PlayerMobile pm = (PlayerMobile)from;
                                ArrayList barracks = new ArrayList();
                                if ( pm.City.Barracks == null )
			        {
                                  pm.City.Barracks = barracks;
                                }
				//Remove List
				...
				CityRecruitStone rStone = new CityRecruitStone();
				rStone.MoveToWorld( new Point3D( p.X + 7, p.Y - 7 , p.Z /*+ 7*/ ), from.Map );
                                rStone.Leader = from;
                                
                                ArrayList decore = pm.City.isLockedDown;
                                ArmyController con = new ArmyController();
                                con.Movable = false;
                                from.AddToBackpack( con );
				con.Owner = from;
    
				sign.toDelete.Add( rStone );
				sign.toDelete.Add( building );
                                sign.toDelete.Add( con );
                                
				sign.Type = CivicSignType.Barracks;

				pm.City.toDelete.Add( sign );
                                pm.City.Barracks.Add( sign );
                                
				sign.ItemID = 3025;
				sign.Name = "City Barracks";
				sign.MoveToWorld( new Point3D( p.X + 7, p.Y - 8 , p.Z  ), from.Map );
			}
			this.Delete();
		}
