...
namespace Server.Items
{
	...
	{ 
		public static void CalculateTokens(Mobile m, BaseCreature creature)
		{
			...
			double d_Resists = ((creature.PhysicalResistance + creature.FireResistance + creature.ColdResistance + creature.PoisonResistance + creature.EnergyResistance)/500000); //set the amount of resists the monster have
			...
			double d_TempTokReward = (i_Hits + ((i_Hits * d_Resists)/100000) ); //set the temp reward
			...
			if (creature.Karma < 0)
				...
				i_FameKarma += creature.Karma;
			i_FameKarma = (i_FameKarma/2500000);
			if (i_FameKarma < 0)
				i_FameKarma = 0;
			...
			{
				double d_Mage = ((creature.Skills[SkillName.Meditation].Value + creature.Skills[SkillName.Magery].Value + creature.Skills[SkillName.EvalInt].Value)/80);
				d_TempTokReward += d_Mage;
			}
							
			if (creature.HasBreath) //give bonus for creatures that have fire breath
			{
				double d_FireBreath = (creature.HitsMax/250000);
			...
			i_TokReward = Utility.RandomMinMax((int)(i_TokReward*0.04), (int)(i_TokReward*0.05));
			if (i_TokReward < 1)
