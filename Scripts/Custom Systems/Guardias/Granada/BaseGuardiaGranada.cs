...
{ 
    public class BaseGuardiaGranada : BaseCreature 
    {
        public override bool BardImmune{ get{ return true; } } 
        public virtual bool AdvancedGuardsCommand{ get{ return true; } }

        public BaseGuardiaGranada(AIType aiType)
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            if (base.Combatant == null)
            {
                ...
                if (InRange(m, base.RangePerception) && InRange(oldLocation, base.RangePerception) && InLOS(m))
                {
                    ...
                    if (base.CanSee(m) && (m is BaseGuardiaNavarra) || !(m.Hidden) && (m.Criminal) && m.AccessLevel == AccessLevel.Player || (m is PlayerMobile && pm.CrimiGranada == true))
                    {
                        base.Combatant = m;
                        base.Warmode = true;
                    }
        ...
        public BaseGuardiaGranada(Serial serial)
            : base(serial) 
        { 
        }
