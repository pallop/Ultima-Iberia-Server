		...
		public virtual void OnCarve(Mobile from, Corpse corpse, Item with)
		{
			...
			else
			{
				...
				if (meat != 0)
				{
										...
										/*else if ( MeatType == MeatType.Rotting )
										{
					    						corpse.AddCarvedItem( new RottingMeat( meat ), from );
										}	*/
...
*/          }
	...
	/*		typeof(Head), typeof(LeftArm), typeof(LeftLeg), typeof(Torso), typeof(RightArm), typeof(RightLeg)
        ...
        {
            typeof(FriedDuckEggs), typeof(FriedChickenEggs), typeof(BrightEggs), typeof(BrightlyColoredEggs),
            typeof(EasterEggs), typeof(HarpyEggs), typeof(FriedEggs), typeof(Eggs)
        };
	...
	private static Type[] m_RottingMeat = new Type[]
			{
...
/*			if ( CheckFoodPreference( f, FoodType.RottingMeat, m_RottingMeat ) )
            {
				return true;
            }
*/
...
/*				case AIType.AI_Necro:
				...
				case AIType.AI_OmniAI:
					...
					break;
           /*  case AIType.AI_Scalis:
					m_AI = new ScalisAI(this);
					break;*/
			...
			// end 2nd
				case AIType.AI_Spellbinder:
					...
					break;
			/*	case AIType.AI_EvoArcher:
					m_AI = new EvoArcherAI(this);
					break;
				case AIType.AI_EvoBerserk:
					m_AI = new EvoBerserkAI(this);
					break;
				case AIType.AI_EvoMage:
					m_AI = new EvoMageAI(this);
					break;
				case AIType.AI_EvoMelee:
					m_AI = new EvoMeleeAI(this);
					break;*/
...
#region FS:ATS Edits
			...
			//{
			//}
			 if ( from.Alive && this.Alive && this.Controlled == true && this.Summoned == false && FSATS.EnablePetLeveling == true )
			{
				bool nolevel = false;
				Type typ = this.GetType();
				string nam = typ.Name;
...
/*//Start GahLakTus edit

            Mobile killerx = this.FindMostRecentDamager(true);

            if (killerx is GahLakTus || killerx is Galactus || killerx is HulkingUndead ||killerx is Spitter || killerx is Sprinter || killerx is Zombiex || killerx is Witch && (this.Map == Map.Trammel))
            {

                //What Should the consuming virus not affect
                if ( this.Summoned || this is GahLakTus || this is Galactus ) 
                ...
                else
                {
                    ...
                    GahLakTus consum = new GahLakTus();

                    consum.Map = this.Map;
                    consum.Female = this.Female;
                    consum.Body = this.Body;
                    consum.Location = this.Location;
                    consum.Hue = 1158;
                    consum.Name = this.Name;
                    consum.Title = "*Infected*";

                    consum.HairItemID = HairItemID;
                    consum.HairHue = HairHue;
                    consum.FacialHairItemID = FacialHairItemID;
                    consum.FacialHairHue = FacialHairHue;

                    dhits = this.HitsMax / 2;
                    dstr = this.Str / 2;
                    for (int i = 0; i < this.Skills.Length; ++i)
                    {
                        consum.Skills[i].Base = this.Skills[i].Base;
                        consum.Skills[i].Cap = this.Skills[i].Cap;
                    }

                    for (int i = 0; i < this.Items.Count; i++)
                    {
                        consum.AddItem(consum.CloneItem(this.Items[i]));
                    }

                    consum.HitsMaxSeed = this.HitsMax + (int)dhits;
                    consum.Hits = this.HitsMax + (int)dhits;
                    consum.DamageMin = this.DamageMin;
                    consum.DamageMax = this.DamageMax;
                    consum.Str = this.Str + (int)dstr;
                    this.Delete();
                    new GahLakTus();
                }
...
ArtifactValidate.ArtiChance(ds.m_Mobile, this);
				...
				//base.OnDeath(c); modificado por loot system
