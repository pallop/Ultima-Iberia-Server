...
namespace Server.Mobiles
{
	...
	public enum PlayerFlag // First 16 bits are reserved for default-distro use, start custom flags at 0x00010000
	{
		...
		//modificacionguardias
		CrimiNavarra = 0x00002000,
        CrimiGranada = 0x00004000,
        CrimiValencia = 0x00008000,
        CrimiLisboa = 0x00010000,
        CrimiSegovia = 0x00020000,
        SinOreja = 0x00040000,
        LenguaCortada = 0x00080000,
        SinBrazoDerecho = 0x00100000,
        SinBrazoIzquierdo = 0x00200000
	}
...
/* End Captcha Mod */
	...
	//System Chance Control
		private int MinorSickness = 	7000; 	//This is the chance of players catching minor sickness (high [4 to 10000] rare).
		private int MajorSickness = 	8000; 	//This is the chance of players catching major sickness (high [3 to 10000] rare).
	
	//System Time Control
		private int SickDelay = 	480; 	//This sets the delay in seconds between sickness damage done to player.
		private int MinSickTime = 	100;	//This is the min counts for sickness to last ( MinSickTime x SickDelay = Total Second min Count )
		private int MaxSickTime = 	240;	//This is the max counts for sickness to last ( MaxSickTime x SickDelay = Total Second max Count )
	...
	//System Damage Control
		...
		private int ColdMax = 		8; 	//This is the max damage done by Cold Sickness.

		private int FluMin = 		5; 	//This is the min damage done by Flu Sickness.
		private int FluMax = 		10; 	//This is the max damage done by Flu Sickness.

		private int VirusMin = 		5; 	//This is the min damage done by Virus Sickness.
		private int VirusMax = 		10; 	//This is the max damage done by Virus Sickness.

		private int PlagueMin = 	8; 	//This is the min damage done by Plague Sickness.
		private int PlagueMax = 	15; 	//This is the max damage done by Plague Sickness.
	
		private int DiseaseMin = 	8; 	//This is the min damage done by Disease Sickness.
		private int DiseaseMax = 	15; 	//This is the max damage done by Disease Sickness.
...
//modificacion guardias
		[CommandProperty(AccessLevel.GameMaster)]
        public bool CrimiValencia
        {
            get { return GetFlag(PlayerFlag.AcceptGuildInvites); }
            set { SetFlag(PlayerFlag.CrimiValencia, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool CrimiGranada
        {
            get { return GetFlag(PlayerFlag.CrimiGranada); }
            set { SetFlag(PlayerFlag.CrimiGranada, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool CrimiLisboa
        {
            get { return GetFlag(PlayerFlag.CrimiLisboa); }
            set { SetFlag(PlayerFlag.CrimiLisboa, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool CrimiSegovia
        {
            get { return GetFlag(PlayerFlag.CrimiSegovia); }
            set { SetFlag(PlayerFlag.CrimiSegovia, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool CrimiNavarra
        {
            get { return GetFlag(PlayerFlag.CrimiNavarra); }
            set { SetFlag(PlayerFlag.CrimiNavarra, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool LenguaCortada
        {
            get { return GetFlag(PlayerFlag.LenguaCortada); }
            set { SetFlag(PlayerFlag.LenguaCortada, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool SinOreja
        {
            get { return GetFlag(PlayerFlag.SinOreja); }
            set { SetFlag(PlayerFlag.SinOreja, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool SinBrazoDerecho
        {
            get { return GetFlag(PlayerFlag.SinBrazoDerecho); }
            set { SetFlag(PlayerFlag.SinBrazoDerecho, value); }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public bool SinBrazoIzquierdo
        {
            get { return GetFlag(PlayerFlag.SinBrazoIzquierdo); }
            set { SetFlag(PlayerFlag.SinBrazoIzquierdo, value); }
        }
        //fin modificacion
	...
	public override void ComputeBaseLightLevels( out int global, out int personal )
{
	...
	if ( this.LightLevel < 21 && AosAttributes.GetValue( this, AosAttribute.NightSight ) > 0 )
		...
		//ORIGINAL ARRIBA MODIFICADO POR PERMADEATH
...
//writer.Write(29); // version old 28
		...
		public override void GetProperties(ObjectPropertyList list)
		{
			...
			if (IsPlayer())
			{
				...
				if (QueenTitle != null && QueenTitle.Data == "false")
				{
					return;
				}
				else
		...
		public override void OnSpeech(SpeechEventArgs e)
		{
			if (SpeechLog.Enabled && NetState != null)
			{
				...
				//añadido ver historial de conversacion
