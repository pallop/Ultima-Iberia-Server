...
{
    public enum RestartType { Daily, Weekly }
    ...
    public class AutoRestart : Timer
    {
        public static void Initialize()
        {
            ...
            CommandSystem.Register("AutoRestartSave", AccessLevel.Administrator, AutoRestartSave_OnCommand);
            CommandSystem.Register("AutoRestartLoad", AccessLevel.Administrator, AutoRestartLoad_OnCommand);
        ...
        private static RestartType RestartFrequency = RestartType.Daily; // The server restarts daily or weekly on a particular day of the week.
        private static DayOfWeek RestartDay = DayOfWeek.Monday; // IF the server restarts weekly, the day of week is set here.
        ...
        {
            public AutoRestartLoadTarget() : base(0, false, TargetFlags.None)
            {
            }
        ...
        private static void AutoRestartSave_OnCommand(CommandEventArgs e)
        {
            if (e.Mobile is PlayerMobile)
            {
                ...
                if (pack == null || pack.Deleted) return;
                pack.DropItem(new AutoRestarter(m_Enabled, m_Restarting, RestartFrequency, RestartDay,RestartMessage,RestartTimeOfDay, WarningColor, RestartDelay, WarningDelays));
                pm.SendMessage("Your settings have been saved to a new AutoRestarter in your pack.");
            }
        }
    ...
    public class AutoRestarter : Item
    {
        ...
        public string WarningDelaysList
        {
            get
            {
                ...
                if (delays.Contains(",")) delays = delays.Remove(delays.Length - 1, 1);
                return delays;
            }
            set
            {
        ...
        public AutoRestarter()
            : this(false, false, RestartType.Daily, DayOfWeek.Monday,
                "The server will be restarting for routine Maintenance", TimeSpan.FromHours(2.0),
                0x22, TimeSpan.Zero, new List<double>() {1.0, 2.0, 5.0, 10.0})
        {
        }

        [Constructable]
        public AutoRestarter(bool enabled, bool restarting, RestartType frequency, DayOfWeek restartDay, string message,
            TimeSpan timeOfDay, int color, TimeSpan delay, List<double> delays) : base(0x1810)
        {
            ...
            m_RestartDateTime = DateTime.UtcNow.Date + m_RestartTimeOfDay;
