...
namespace Server.Items
{
    ...
    public abstract class BaseClothing : WearableItem, IDyable, IScissorable, IFactionItem, ICraftable, IWearableDurability, ISetItem
    {
        #region Factions
            ...
            if (from.IsPlayer())
            {
                ...
                else if (this.RequiredRace != null && from.Race != this.RequiredRace)
                {
                    if (this.RequiredRace == Race.Elf)
                        from.SendMessage("Humanos"); // Only Elves may use this.
